...
@section('content')
  <div class="container">
    <h1 class="title">{{ __('Editar Partit Jornada: :jornada', ['jornada' => $partit->jornada]) }}</h1>
    ...
    <form action="{{ route('partits.update', $partit) }}" method="POST" class="form-container">
      @csrf
      @method('PUT')
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="form-group">
          ...
          <input type="datetime-local" name="data"
            value="{{ old('data', \Carbon\Carbon::parse($partit->data)->format('Y-m-d\TH:i')) }}" class="form-input">
        </div>
        <div class="form-group">
          <label class="form-label">{{ __('Jornada') }}:</label>
          <input type="number" name="jornada" value="{{ old('jornada', $partit->jornada) }}" class="form-input">
        </div>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="form-group">
          <label class="form-label">{{ __('Gols Local') }}:</label>
          <input type="number" name="gols_local" value="{{ old('gols_local', $partit->gols_local) }}" class="form-input">
        </div>
        <div class="form-group">
          <label class="form-label">{{ __('Gols Visitant') }}:</label>
          <input type="number" name="gols_visitant" value="{{ old('gols_visitant', $partit->gols_visitant) }}"
            class="form-input">
        </div>
      </div>

      <div class="flex gap-2 pt-2">
        <button type="submit" class="btn--submit">{{ __('Actualitzar Partit') }}</button>
